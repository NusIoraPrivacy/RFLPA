...
def model_dist_norm_var(model, target_params_variables, norm=2):
    ...
    for name, layer in model.named_parameters():
        nr += (torch.norm(layer - target_params_variables[name], 2))**2
    return nr

def compute_krum_attack(weights, global_weight, s, args, thd=1e-5, max_itr=100):
    non_malicious_count = max(len(weights) - int(args.attack_prop * len(weights)), 1)
    # compute initial upper bound
    d = 0
    for key in global_weight:
        d += global_weight[key].numel()
    lambda_1 = 0
    base_weight = weights[0]
    for weight in weights[1:]:
        dist = 0
        for key in weight:
            dist += torch.sum((weight[key] - base_weight[key]) ** 2).float()
        dist = torch.sqrt(dist)
        lambda_1 += dist.item()
    lambda_1 = 1 / math.sqrt(d) * lambda_1
    lambda_2 = 0
    for weight in weights:
        dist = 0
        for key in weight:
            dist += torch.sum((weight[key] - global_weight[key]) ** 2).float()
        dist = torch.sqrt(dist)
        lambda_2 = max(dist.item(), lambda_2)
    lambda_2 = 1 / math.sqrt(d) * lambda_2
    _lambda = lambda_1 + lambda_2

    est_weights = {}
    for key in global_weight:
        est_weights[key] = global_weight[key] - _lambda * s[key]
    ...
    while(_lambda > thd):
        ...
        for key in global_weight:
            est_weights[key] = global_weight[key] - _lambda * s[key]
        w = [est_weights] + weights
        distances = np.zeros((len(w), len(w)))
        for i in range(len(w)):
            for j in range(i):
                for key in global_weight:
                    other_weight = w[j][key]
                    this_weight = w[i][key]
                    this_dist = (torch.sum((other_weight - this_weight)** 2)).float()
                    distances[i,j] +=  this_dist.item()
                    distances[j,i] = distances[i,j]
        distances = np.sqrt(distances)
        distances = np.sort(distances)
        distances = distances[:,:non_malicious_count]
        sorted_idx = distances.sum(axis=1).argsort()[:non_malicious_count]
        # print(sorted_idx)
        if 0 in sorted_idx or len(weights) == 1:
            break
        _lambda = _lambda / 2
        cur_itr += 1
    return est_weights
